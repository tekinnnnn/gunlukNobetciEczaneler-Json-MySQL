...
$bugunkontrol = $DB->query("SELECT tarih FROM eczaneler ORDER BY id DESC LIMIT 1");
$buguncekildimi = $bugunkontrol->fetch();
if ($buguncekildimi["tarih"] != date("Y-m-d")) {
    $DB->exec("SET names utf8");
    
    for ($ilce = 0; $ilce < count($ilcelerveri); $ilce ++) {
        
        $url = $ilcelerveri[$ilce][1];
        preg_match_all('/<table>(.*?)<\/table>/is', curl($url), $veri_derece_2);
        ...
        for ($eczane = 0; $eczane < count($veri_derece_3[1]); $eczane ++) {
            $ilceId = $ilce + 1;
            $query = $DB->exec("INSERT INTO eczaneler VALUES (null,'" . $ilceId . "','" . $veri_derece_3[1][$eczane] . "','" . $veri_derece_3[2][$eczane] . "','" . $veri_derece_3[3][$eczane] . "','" . $veri_derece_3[4][$eczane] . "','" . $veri_derece_3[5][$eczane] . "', now(), null)");
            // sleep(1);
        }
    }
}
